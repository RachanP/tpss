{{-- ฟิลด์เทอม 1 บล็อก (ใช้ index ตายตัว — name ต้อง static) · V2 Master Data Cleanup
     เรียกผ่าน @include(['index'=>..,'seq'=>..,'label'=>.., 'model'=>'currentCal.terms'(optional)])
     model = root ของ Alpine x-model (default = currentYear.terms · ปฏิทินตามกลุ่มใช้ currentCal.terms) --}}
@php($model = $model ?? 'currentYear.terms')
<div style="border:1px solid var(--border);border-radius:8px;padding:12px 14px;margin-bottom:10px;background:var(--surface);">
    ...
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;">
        ...
        <input type="text" name="terms[{{ $index }}][name]" x-model="{{ $model }}[{{ $index }}].name"
            placeholder="{{ $label }}"
    ...
    <div class="form-row">
        <div class="form-group">
            ...
            <x-thai-date-input name="terms[{{ $index }}][start_date]" x-model="{{ $model }}[{{ $index }}].start_date" helper="" />
        </div>
        <div class="form-group">
            <label style="font-size:12px;">วันสิ้นสุดเทอม</label>
            <x-thai-date-input name="terms[{{ $index }}][end_date]" x-model="{{ $model }}[{{ $index }}].end_date" helper="" />
        </div>
    </div>
    <div class="form-row" style="margin-top:8px;">
        <div class="form-group">
            <label style="font-size:12px;color:var(--fg-3);">สอบกลางภาค — เริ่ม</label>
            <x-thai-date-input name="terms[{{ $index }}][midterm_start]" x-model="{{ $model }}[{{ $index }}].midterm_start" helper="" />
        </div>
        <div class="form-group">
            <label style="font-size:12px;color:var(--fg-3);">สอบกลางภาค — สิ้นสุด</label>
            <x-thai-date-input name="terms[{{ $index }}][midterm_end]" x-model="{{ $model }}[{{ $index }}].midterm_end" helper="" />
        </div>
    </div>
    <div class="form-row" style="margin-top:8px;">
        <div class="form-group">
            <label style="font-size:12px;color:var(--fg-3);">สอบปลายภาค — เริ่ม</label>
            <x-thai-date-input name="terms[{{ $index }}][final_start]" x-model="{{ $model }}[{{ $index }}].final_start" helper="" />
        </div>
        <div class="form-group">
            <label style="font-size:12px;color:var(--fg-3);">สอบปลายภาค — สิ้นสุด</label>
            <x-thai-date-input name="terms[{{ $index }}][final_end]" x-model="{{ $model }}[{{ $index }}].final_end" helper="" />
        </div>
    </div>
</div>
