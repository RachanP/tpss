...
class CourseOfferingShowPageTest extends TestCase
{
    ...
    // ── practicum_note override flow ─────────────────────────────────

    public function test_update_without_override_does_not_require_practicum_note(): void
    {
        $head     = $this->makeUser('course_head');
        $course   = $this->makeCourse(['requires_practicum_rotation' => false]);
        $offering = $this->makeOffering($head, $course, ['requires_practicum_rotation' => false]);
        ...
        $this->assertNull($offering->fresh()->practicum_note);
    }

    public function test_update_overriding_course_default_requires_practicum_note(): void
    {
        $head     = $this->makeUser('course_head');
        $course   = $this->makeCourse(['requires_practicum_rotation' => false]);
        $offering = $this->makeOffering($head, $course, ['requires_practicum_rotation' => false]);
        ...
        $this->from(route('maker.course_offerings.show', $offering))
            ->put(route('maker.course_offerings.update', $offering), [
                ...
                // practicum_note omitted
            ])
            ->assertSessionHasErrors('practicum_note');

        $this->assertFalse((bool) $offering->fresh()->requires_practicum_rotation);
    }

    public function test_update_with_override_saves_practicum_note(): void
    {
        $head     = $this->makeUser('course_head');
        $course   = $this->makeCourse(['requires_practicum_rotation' => true]);
        $offering = $this->makeOffering($head, $course, ['requires_practicum_rotation' => true]);

        $this->actingAsCourseHead($head);
        $this->from(route('maker.course_offerings.show', $offering))
            ->put(route('maker.course_offerings.update', $offering), [
                'requires_practicum_rotation' => 0,
                'practicum_note'              => '  ปีนี้ยกเลิกการหมุนเวียน  ',
            ])
        ...
        // Note value is trimmed by the controller
        $this->assertSame('ปีนี้ยกเลิกการหมุนเวียน', $fresh->practicum_note);
    }

    public function test_update_clears_practicum_note_when_returning_to_default(): void
    {
        $head     = $this->makeUser('course_head');
        $course   = $this->makeCourse(['requires_practicum_rotation' => true]);
        $offering = $this->makeOffering($head, $course, [
            'requires_practicum_rotation' => false,
            'practicum_note'              => 'previous override',
        ]);
