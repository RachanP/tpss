...
## 🆕 Requirement V4 Update — 🔲 PROPOSED (จากประชุมลูกค้า · ยังไม่ implement)

> แหล่งอ้างอิงเต็ม: `Doc/requirement/requirement_v4.md` (ข้อ 13 = สรุปรวม 7 ข้อ)
> ยังไม่ลงมือ — ห้ามถือว่ามีในโค้ดจนกว่าจะมี commit จริง · บางข้อ **กลับทิศ**จาก decision เดิมด้านล่าง

7 ข้อใหม่จากการประชุมลูกค้า:

1. **หัวข้อกิจกรรมสำเร็จรูป (dropdown)** — Admin กรอกหัวข้อกิจกรรม/บทเรียนของวิชาไว้ล่วงหน้า → หัวหน้าวิชากดเลือกได้เลย ไม่พิมพ์ใหม่ (วิชาใช้หัวข้อซ้ำ) · master data ใหม่ "activity topic" ต่อวิชา + dropdown ในฟอร์ม slot (ยังอนุญาต free text)
2. ✅ **เคาะแล้ว (4 มิ.ย.): หัวหน้าวิชาจัดกลุ่มนักศึกษาตั้งแต่แรก — กลับทิศจาก decision 30–31 พ.ค.** ที่ว่า "หัวหน้าวิชาไม่จัดกลุ่ม · subgroup เกิดหลังอนุมัติโดยอาจารย์". V4 ยืนยันว่า **หัวหน้าวิชาจัดทั้งกิจกรรม + กลุ่มย่อยตั้งแต่ตอนกรอกตาราง** โดย **ฐาน = จำนวน นศ. ในชั้นปีของปีนั้น ไม่ใช่ capacity วิชา** → map กลับเป็นตารางเรียนรายกลุ่มได้. ผลกระทบ: **นำหน้าจัดกลุ่มย่อยกลับเข้า course head** (ที่เคยถอดออก), ทบทวน capacity gate (เทียบจำนวนชั้นปี ไม่ใช่ capacity_required), cross-course group conflict ขยับมา *ก่อน* อนุมัติ. **เพิ่มเติม 4 มิ.ย.:** ปี 3-4 = **4 กลุ่มใหญ่** (ไม่ใช่ 2 A/B) · rotation = ปีละ 4 รอบ (มิดเทอมเป็นเส้นแบ่งในแต่ละเทอม) + สลับข้ามเทอม
3. **ผู้บริหาร = หัวหน้าภาควิชา** — เฉพาะ role `executive` (และ `admin`) เท่านั้นกำหนดตำแหน่งหัวหน้าภาคได้ · `instructor` ทำไม่ได้ → ต้อง gate ฟังก์ชันนี้
4. **หลักสูตรนับงานบริการวิชาการอย่างเดียว** — หลักสูตรเฉพาะทางบางตัวไม่นับชั่วโมงทำการสอนปกติ → เพิ่ม flag ประเภทหลักสูตร (เกี่ยว Module Workload เฟสถัดไป)
5. **อาจารย์กรอกสัดส่วน PA เอง** — instructor ล็อกอินกรอกสัดส่วนภาระงานตนเอง แทน admin กรอกรวม (เปลี่ยนทุกรอบประเมิน) → หน้า self-service + เก็บรอบประเมิน · validation 100% + min/max เดิมคงไว้ (ดู PA Criteria)
6. **สลับเวร/แลกคลาส ผ่านหัวหน้าวิชาเท่านั้น** — หัวหน้าวิชาเปลี่ยนชื่อ instructor ใน slot ให้ (ไม่ตกลงกันเองนอกระบบ) เพื่อโอนชั่วโมง/PA ไปคนที่สอนแทนถูกต้อง · หน้าตารางต้องแสดง **last-updated timestamp** + audit trail
7. **สร้างกิจกรรมแบบลากช่วงวันที่ (Start–End)** — คลุมหลายสัปดาห์ครั้งเดียวสำหรับวิชาปฏิบัติต่อเนื่อง · ต่อยอด `schedule_templates`/`storeSeries` · ข้ามวันหยุด/สอบ/ปิดเทอมอัตโนมัติ (ใช้ `AcademicCalendar`)
...
2. ✅ **RESOLVED (4 มิ.ย.): ปี 3-4 = 4 กลุ่มใหญ่** (ไม่ใช่ 2 กลุ่ม A/B ตามเอกสารพิม) — `student_cohorts` รองรับได้ ปรับ seeder/ตัวอย่างให้เป็น 4 กลุ่ม
