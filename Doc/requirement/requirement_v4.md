# สรุปขอบเขตและข้อกำหนดของระบบ TPSS (ฉบับ V4)

เอกสารนี้สรุปจากไฟล์ `รายละเอียดระบบ_V1ล่าสุด.docx`, เอกสาร V3 และ **ข้อกำหนดเพิ่มเติมจากการประชุมกับลูกค้า (รอบ V4)** เพื่อใช้เป็นเอกสารทำความเข้าใจขอบเขตของระบบจัดตารางสอนและการฝึกปฏิบัติ (Teaching & Practicum Scheduling System: TPSS)

> **🆕 อ่านก่อน — สรุปสิ่งที่เปลี่ยนใน V4**: เอกสารฉบับนี้เพิ่มข้อกำหนดใหม่ 7 ข้อจากการประชุมลูกค้า ดูสรุปรวมทั้งหมดที่ **ข้อ 13** ท้ายเอกสาร · ส่วนที่เปลี่ยน/เพิ่มในแต่ละหัวข้อจะมีป้าย **🆕 V4** กำกับ
>
> **⚠️ การเปลี่ยนทิศที่สำคัญที่สุด**: ข้อกำหนดใหม่ระบุว่า **หัวหน้าวิชาเป็นผู้จัดกลุ่มนักศึกษา (subgroup) ตั้งแต่ต้น** พร้อมกับการจัดกิจกรรม — *กลับทิศ*จากแนวทางเดิม (V3 / 30–31 พ.ค.) ที่ให้กลุ่มย่อยเกิดหลังอนุมัติโดยอาจารย์ · และฐานการแบ่งกลุ่มเปลี่ยนจาก "จำนวนเปิดรับของวิชา (capacity)" → **"จำนวนนักศึกษาในชั้นปีของปีนั้น ๆ"** · ดูข้อ 5.5 และข้อ 13
...
> **🆕 V4 (เคาะ 4 มิ.ย.) — จำนวนกลุ่ม + รอบ rotation**:
> - **ปี 3-4 = 4 กลุ่มใหญ่** (ไม่ใช่ 2 กลุ่ม A/B ตามตัวอย่างเดิมด้านล่าง — ตัวอย่าง A/B ยังใช้อธิบาย *แนวคิด* การสลับได้ แต่จำนวนจริง = 4)
> - **รอบ rotation = ทั้งสองแบบรวมกัน**: ปีละ **4 รอบ** โดยใช้ **สอบมิดเทอมเป็นเส้นแบ่งรอบในแต่ละเทอม** (เทอมละ 2 รอบ) **+ สลับกลุ่มข้ามเทอม** (semester swapping)
...
3. **🆕 V4 — ปฏิทินการศึกษาต่างกันตามหลักสูตร/ชั้นปี (Per-group Academic Calendar)**:
   - **ใน 1 ปีการศึกษา มีปฏิทินได้หลายชุด** — วันเปิด/ปิดเทอม และช่วงสอบกลางภาค/ปลายภาค **ไม่จำเป็นต้องเหมือนกันทุกกลุ่ม**
   - ต่างกัน **2 มิติ**:
     1. **ตามหลักสูตร**: ป.ตรี / ป.โท / ป.เอก / หลักสูตรนานาชาติ เปิด-ปิดเทอม + ช่วงสอบ คนละช่วง
     2. **ตามชั้นปีในหลักสูตรเดียวกัน**: เช่น ป.ตรี ปี 1-2 กับ ปี 3-4 — เทอม 1 อาจสอบช่วงเดียวกัน แต่เทอม 2 ปี 3-4 ขึ้นฝึก rotation ทำให้ช่วงสอบเลื่อนไม่ตรงกับปี 1-2
   - **โครงสร้าง (เคาะ — ทำแบบเต็ม "ปฏิทินแยกตามกลุ่ม")**: เพิ่ม entity **`academic_calendars`** (ปฏิทินย่อยภายใต้ปีการศึกษา) — แต่ละปฏิทินมีชุด `terms` (วันเปิด/ปิด + ช่วงสอบ) ของตัวเอง · แต่ละปฏิทินผูกขอบเขต (หลักสูตร + ช่วงชั้นปี) เพื่อให้ระบบ resolve ว่ากิจกรรม/กลุ่มไหนใช้ปฏิทินใด · มีปฏิทิน default รองรับกรณีไม่ระบุ
   - **ผลกระทบ**: `terms` ย้ายไปสังกัด `academic_calendars` (จากเดิมสังกัด `academic_years` ตรง ๆ) · `AcademicCalendar` service ต้อง resolve ปฏิทินตามหลักสูตร/ชั้นปีของ slot · หน้าตั้งค่าปีการศึกษาเพิ่มการจัดการหลายปฏิทิน · กระทบการบล็อก slot ช่วงสอบ/ปิดเทอมของ Branch A
   - ดูสรุปข้อ 13 (#8) + design schema ใน `.claude/rules/database.md`
...
- **🆕 V4 — สร้างกิจกรรมแบบลากช่วงวันที่ (Date Range)**: รองรับการเลือกวันเริ่ม–วันสิ้นสุดเพื่อสร้างกิจกรรมคลุมหลายสัปดาห์ในครั้งเดียว สำหรับวิชาปฏิบัติที่ขึ้นหอผู้ป่วยต่อเนื่อง (ดูข้อ 4.3)
- **🆕 V4 — เลือกหัวข้อกิจกรรมจากรายการสำเร็จรูป (Dropdown)**: ถ้า Admin กรอกหัวข้อกิจกรรม/บทเรียนของวิชานั้นไว้ล่วงหน้า หัวหน้าวิชากดเลือกได้เลย ไม่ต้องพิมพ์ใหม่ (ดูข้อ 5.6)
- แก้ไข ลบ และเปลี่ยนสถานะรายการตารางตามสิทธิ์ พร้อม **🆕 V4 แสดงเวลาที่แก้ไขล่าสุด (Last Updated Timestamp)** บนหน้าตาราง (ดูข้อ 7 ขั้นที่ 12)
...
### 4.3 🆕 V4 — การสร้างกิจกรรมแบบลากช่วงวันที่ (Date Range Bulk Entry)

วิชาภาคปฏิบัติที่ต้องขึ้นหอผู้ป่วยต่อเนื่องยาว ๆ (เช่น ฝึก 4–8 สัปดาห์ติด) ปัจจุบันต้องคลิกสร้างกิจกรรมทีละวันซ้ำ ๆ ซึ่งช้าและเสียเวลา ระบบจึงต้องให้หัวหน้าวิชาเลือก **วันเริ่ม (Start Date) – วันสิ้นสุด (End Date)** เพื่อสร้างกิจกรรมรูปแบบเดียวกันคลุมทั้งช่วงในครั้งเดียว

**ข้อกำหนด**:
- เลือกช่วงวันที่ + เวลา + กลุ่ม + สถานที่ + ผู้สอน + ประเภทกิจกรรม → ระบบสร้างรายการลูก (instance) ให้ทุกวันในช่วงโดยอัตโนมัติ
- รองรับการเลือก **เฉพาะวันในสัปดาห์ที่ต้องการ** (เช่น จ.–ศ. ข้ามเสาร์–อาทิตย์) ได้
- ระบบ **ข้ามวันหยุดราชการ / วันสอบ / วันปิดภาคเรียน** อัตโนมัติ (ตามข้อ 2.4) — หรือแจ้งเตือนให้ผู้ใช้ตัดสินใจ
- ตรวจ Conflict/Warning ของทุกรายการที่ถูกสร้างเหมือนการกรอกทีละรายการ
- แก้ไขรายการลูกแต่ละวันได้ภายหลัง (เช่น เปลี่ยนห้อง/กลุ่มเฉพาะบางวัน) โดยไม่กระทบทั้งชุด

> ต่อยอดจากฟังก์ชัน Series/Template ที่มีอยู่เดิม (`schedule_templates`) — เพิ่มมุมมอง "ลากช่วงวันที่" ที่ใช้ง่ายขึ้นสำหรับ block ฝึกปฏิบัติต่อเนื่อง
...
#### 🆕 V4 — หลักสูตรประเภท "นับงานบริการวิชาการอย่างเดียว"

คณะพยาบาลฯ มี **หลักสูตรเฉพาะทาง (เช่น หลักสูตรการพยาบาลเฉพาะทาง / อบรมระยะสั้น)** ที่ภาระงานสอนจะถูกนับเป็น **"งานบริการวิชาการ"** เท่านั้น **ไม่นับเป็นชั่วโมงทำการสอนปกติ** เหมือนหลักสูตรปริญญาทั่วไป

**ข้อกำหนด**:
- หลักสูตรต้องมี metadata/flag ระบุว่าเป็นหลักสูตรที่ **นับงานบริการวิชาการอย่างเดียว** (ไม่นับชั่วโมงทำการปกติ)
- เมื่อคำนวณภาระงาน (Module Workload — ยังไม่พัฒนา) กิจกรรมของหลักสูตรประเภทนี้ต้องถูกแยกไปหมวด "บริการวิชาการ" ไม่รวมในชั่วโมงสอนปกติ
- เป็นข้อมูลที่ต้องเตรียมโครงสร้างให้พร้อมตั้งแต่ตอนนี้ แม้ตัวการคำนวณภาระงานจะอยู่เฟสถัดไป

> เกี่ยวข้องกับ **Module Workload** ที่ยังไม่ได้ทำ — V4 เพียงระบุว่าโครงสร้างหลักสูตรต้องรองรับการจำแนกประเภทนี้ ดูความเชื่อมโยงกับประเภทกิจกรรม "นับ/ไม่นับภาระงาน" ในข้อ 5.4
...
> **🆕 V4 (เคาะ 4 มิ.ย.) — เลขานุการวิชา**: อยู่ใน **template ระดับวิชา (course-level)** เช่นเดียวกับอาจารย์คนอื่น แต่ **หัวหน้าวิชาแก้ไขได้** ที่ระดับ offering (snapshot) — ไม่ใช่ field แยกต่างหาก ใช้บทบาท (course role) "เลขานุการวิชา" ใน instructor pool
...
#### 🆕 V4 — ใครจัดกลุ่ม และจัดอย่างไร (เปลี่ยนทิศจาก V3)

> **⚠️ ข้อนี้กลับทิศจากแนวทางเดิม (V3 / การเคาะ 30–31 พ.ค.)** ที่ระบุว่า "หัวหน้าวิชาไม่ยุ่งกับกลุ่มนักศึกษา และกลุ่มย่อย (subgroup) เกิดหลังอนุมัติโดยอาจารย์" — จากการประชุมลูกค้ารอบ V4 ยืนยันใหม่ว่า:

1. **หัวหน้าวิชาเป็นผู้จัดทั้งกิจกรรมและกลุ่มนักศึกษาตั้งแต่แรก** (ในขั้นกรอกตาราง ก่อนส่งอนุมัติ) — ไม่ใช่งานที่เกิดหลังอนุมัติ
2. **ฐานการแบ่งกลุ่ม = จำนวนนักศึกษาในชั้นปีของวิชานั้น ๆ ของปีการศึกษานั้น** — *ไม่ใช่* จำนวนเปิดรับ/ความจุของรายวิชา (capacity)
   - เช่น ปี 3 ปีการศึกษา 2568 มี 270 คน → หัวหน้าวิชาแบ่งเป็นกลุ่มย่อย (A1–A9 ฯลฯ) ตามจำนวนจริงของชั้นปีนั้น
3. **ผลลัพธ์ = ตารางเรียนรายกลุ่มนักศึกษา**: เมื่อนำทุกวิชาของแต่ละกลุ่มมา map กลับ จะได้ตารางเรียนของกลุ่มนั้นครบ
   - กิจกรรมของกลุ่มย่อยต่างกันได้ในวิชาเดียวกัน เพราะอยู่คนละ subgroup (เช่น A1 ขึ้นวอร์ดเพศชาย / A2 conference ในวันเดียวกัน)

```
ตัวอย่าง map กลับเป็นตารางเรียนรายกลุ่ม:

กลุ่ม A1 (30 คน) ของปี 3 — รวมทุกวิชาที่ A1 ลงเรียนในเทอมนี้
├── NSBS 301: จ.–พ. ฝึกหอเพศชาย รพ.ศิริราช (อ.สมศรี)
├── NSBS 311: พฤ. บรรยาย ห้อง N301 (อ.มานะ)
└── NSBS 320: ศ. SDL
        ↑ ตารางเรียนของ A1 เกิดจากการรวมกิจกรรมข้ามวิชาที่ผูกกับ A1
```

**ผลกระทบเชิงระบบ (สำหรับวางแผนพัฒนา)**:
- **capacity gate** ที่เคยบังคับ `sum(student_count) ≤ capacity_required` ตอน save slot — ต้องทบทวน เพราะฐานเปลี่ยนเป็น "จำนวน นศ. ในชั้นปี" ไม่ใช่ความจุวิชา
- หน้าจัดการกลุ่มย่อยของหัวหน้าวิชา (ที่เคยถอดออกตามแนว V3) **ต้องนำกลับเข้ามา** ในขั้นจัดตาราง
- ต้องมีฐานข้อมูล "จำนวนนักศึกษาชั้นปีต่อปีการศึกษา" (เชื่อมกับ `student_cohorts`) เป็นตัวตั้งของการแบ่งกลุ่ม
- cross-course **group conflict** (กลุ่มเดียวกันห้ามถูกจัด 2 วิชาพร้อมกัน) กลายเป็นเรื่องจริงตั้งแต่ขั้นจัดตาราง ไม่ใช่หลังอนุมัติ

### 5.6 🆕 V4 — หัวข้อกิจกรรมสำเร็จรูป (Activity Topic Templates แบบ Dropdown)

หลายวิชามักใช้ **หัวข้อกิจกรรม/บทเรียน (เช่น "ระบบหัวใจและหลอดเลือด", "Pre-conference", "เยี่ยมบ้าน")** ซ้ำ ๆ ทุกปี การพิมพ์ใหม่ทุกครั้งทั้งช้าและทำให้ชื่อหัวข้อไม่สม่ำเสมอ

**ข้อกำหนด**:
- **Admin สามารถกรอกรายการหัวข้อกิจกรรม/บทเรียนของวิชานั้นไว้ล่วงหน้าได้** (ผูกกับรายวิชา หรือกับประเภทกิจกรรม)
- เมื่อวิชาใดมีหัวข้อที่ Admin เตรียมไว้แล้ว → ตอนหัวหน้าวิชากรอกตาราง ช่อง "หัวข้อกิจกรรม" จะเป็น **Dropdown ให้กดเลือกได้เลย ไม่ต้องพิมพ์ใหม่**
- ยังควรอนุญาตให้พิมพ์หัวข้อเองได้ (free text) สำหรับกิจกรรมที่ไม่อยู่ในรายการ
- ช่วยลดการกรอกซ้ำและทำให้ชื่อหัวข้อมาตรฐานเดียวกัน — สอดคล้องกับเป้าหมาย "กรอกตารางที่ซับซ้อนได้ง่าย ลดข้อมูลซ้ำ"

> หมายเหตุ: "หัวข้อกิจกรรม" (topic/บทเรียน) เป็นคนละชั้นกับ "ประเภทกิจกรรม" (activity type เช่น บรรยาย/ฝึกปฏิบัติ ในข้อ 5.4) — หัวข้อคือเนื้อหาย่อยภายใต้ประเภทกิจกรรมหนึ่ง ๆ
...
### 6.1 🆕 V4 — ผู้บริหารคือหัวหน้าภาควิชา และสิทธิ์การกำหนดตำแหน่งหัวหน้าภาค

**ผู้บริหาร (Executive) = หัวหน้าภาควิชานั้น ๆ** (เช่น หัวหน้าภาควิชาการพยาบาลอายุรศาสตร์ เป็นผู้อนุมัติตารางของวิชาในภาคนั้น)

**ข้อกำหนดสิทธิ์**:
- การกำหนด/แต่งตั้ง **ตำแหน่งหัวหน้าภาควิชา** ทำได้เฉพาะผู้ที่มี **บทบาทผู้บริหาร (Executive)** เท่านั้น
- **อาจารย์ทั่วไป (Instructor) ไม่มีสิทธิ์เพิ่มหรือกำหนดตำแหน่งหัวหน้าภาควิชา** — ป้องกันการแต่งตั้งตัวเองหรือผู้อื่นโดยไม่มีอำนาจ
- ระบบต้อง gate การเข้าถึงฟังก์ชันเลือก/กำหนดหัวหน้าภาคไว้ที่ role executive (และ admin สำหรับงานตั้งค่าระบบ)

### 6.2 🆕 V4 — อาจารย์กรอกสัดส่วนภาระงาน (PA) ด้วยตนเอง

เดิมการกรอกสัดส่วนภาระงาน PA ของอาจารย์แต่ละคน (สอน/วิจัย/บริการ/ทำนุบำรุงศิลปะ/อื่น ๆ) ทำโดย Admin กรอกรวมให้ทีเดียว ซึ่ง **เป็นภาระหนักของ Admin** เพราะสัดส่วนเปลี่ยน **ทุกรอบการประเมิน PA**

**ข้อกำหนด**:
- **อาจารย์แต่ละคนเข้ามากรอกสัดส่วนภาระงานของตนเองได้** โดยล็อกอินด้วยรหัสตัวเอง
- ลดภาระ Admin ที่ต้องกรอกแทนทุกคนทุกรอบ
- ระบบยังคงตรวจความถูกต้อง (ผลรวม = 100% และแต่ละด้านอยู่ในเกณฑ์ min/max ของกลุ่มตำแหน่งนั้น) ตามที่มีอยู่
- เก็บประวัติ/รอบการประเมิน เพื่อให้แต่ละรอบ PA มีสัดส่วนของตัวเอง (รายละเอียดการคำนวณภาระงานยังเป็นเฟสถัดไป)

> เกี่ยวข้องกับ **Module Workload / PA** ที่ยังไม่พัฒนาเต็มรูปแบบ — V4 ระบุ "วิธีกรอกข้อมูล" (self-service) ที่ต้องรองรับ
...
### 7.1 🆕 V4 — การสลับเวร/แลกคลาส และการแสดงเวลาแก้ไขล่าสุด

**กรณีฉุกเฉินหรืออาจารย์ขอสลับเวร/แลกคลาสกันสอน**:
- ให้ **หัวหน้าวิชาเป็นผู้เข้ามาเปลี่ยนชื่ออาจารย์ผู้สอนในระบบให้** — *ห้าม*ให้อาจารย์ไปตกลงแลกกันเองนอกระบบ
- **เหตุผล**: เพื่อให้ชั่วโมงสอนและภาระงานของเวร/คลาสนั้น **โอนไปเป็นคะแนน PA ของอาจารย์ที่มาสอนแทนได้อย่างถูกต้อง** (ถ้าแลกกันเองนอกระบบ ระบบจะนับภาระงานให้คนเดิม ผิดความจริง)

**การแสดงเวลาแก้ไขล่าสุด (Last Updated Timestamp)**:
- หน้าตารางต้องแสดง **เวลาที่ตารางถูกแก้ไขล่าสุด** อย่างชัดเจน
- เพื่อให้ **หัวหน้าวิชาและผู้บริหาร** รู้ว่าตารางนี้ถูกปรับล่าสุดเมื่อไหร่ — สำคัญมากในช่วง operation phase ที่มีการแก้บ่อย
- ควรเก็บคู่กับประวัติการแก้ไข (audit trail) เพื่อ traceability ตามมาตรฐาน ISO/IEC 29110
...
## 13. 🆕 ข้อกำหนดใหม่จากการประชุมลูกค้า (สรุปรวม V4)

ข้อกำหนด 8 ข้อต่อไปนี้มาจากการประชุม/ทบทวนเอกสารกับลูกค้ารอบล่าสุด และถูกแทรกไว้ในหัวข้อที่เกี่ยวข้องด้านบนแล้ว (ป้าย 🆕 V4) ตารางนี้ใช้เป็น index สรุป:

| # | ข้อกำหนดใหม่ | สาระสำคัญ | อยู่ในหัวข้อ | ผลกระทบ |
|---|------------|-----------|-------------|---------|
| 1 | **หัวข้อกิจกรรมสำเร็จรูป (Dropdown)** | Admin กรอกหัวข้อกิจกรรม/บทเรียนของวิชาไว้ล่วงหน้า → หัวหน้าวิชากดเลือกจาก dropdown ได้เลย ไม่ต้องพิมพ์ใหม่ (วิชามักใช้หัวข้อเดิมซ้ำ) | ข้อ 4.1, **5.6** | เพิ่ม master data หัวข้อกิจกรรมต่อวิชา + UI dropdown ในฟอร์มกรอกตาราง |
| 2 | **หัวหน้าวิชาจัดกลุ่มตั้งแต่แรก (ฐาน = จำนวน นศ. ชั้นปี)** | หัวหน้าวิชาจัดกิจกรรม + กลุ่มนักศึกษาตั้งแต่ต้น โดยอิง **จำนวน นศ. ในชั้นปีของปีนั้น** ไม่ใช่ capacity วิชา → map กลับเป็นตารางเรียนรายกลุ่มได้ | **5.5** | ⚠️ **กลับทิศจาก V3** — นำหน้าจัดกลุ่มย่อยกลับเข้า course head, ทบทวน capacity gate, group conflict ขยับมาก่อนอนุมัติ |
| 3 | **ผู้บริหาร = หัวหน้าภาค + สิทธิ์กำหนดหัวหน้าภาค** | Executive คือหัวหน้าภาควิชา · เฉพาะ role executive (และ admin) เท่านั้นที่กำหนดตำแหน่งหัวหน้าภาคได้ · อาจารย์ทั่วไปทำไม่ได้ | **6.1** | เพิ่ม permission gate ที่ฟังก์ชันกำหนดหัวหน้าภาค |
| 4 | **หลักสูตรนับงานบริการวิชาการอย่างเดียว** | หลักสูตรเฉพาะทางบางหลักสูตรนับเป็นงานบริการวิชาการ ไม่นับชั่วโมงทำการสอนปกติ | **5.1** | เพิ่ม flag ประเภทหลักสูตร · เกี่ยวกับ Module Workload (เฟสถัดไป) |
| 5 | **อาจารย์กรอกสัดส่วน PA เอง** | อาจารย์แต่ละคนล็อกอินกรอกสัดส่วนภาระงาน PA ของตนเอง แทน Admin กรอกรวม (สัดส่วนเปลี่ยนทุกรอบประเมิน) | **6.2** | เพิ่มหน้า self-service ให้ instructor + เก็บรอบประเมิน |
| 6 | **หัวหน้าวิชาจัดการสลับเวร + timestamp** | สลับเวร/แลกคลาส → หัวหน้าวิชาเปลี่ยนชื่ออาจารย์ในระบบให้ (ไม่ตกลงกันเองนอกระบบ) เพื่อโอน PA ถูกคน · หน้าตารางแสดงเวลาแก้ไขล่าสุด | **7.1** | audit trail + last-updated timestamp บนหน้าตาราง |
| 7 | **สร้างกิจกรรมแบบลากช่วงวันที่** | เลือก Start–End Date คลุมหลายสัปดาห์ในครั้งเดียว สำหรับวิชาปฏิบัติขึ้นหอผู้ป่วยต่อเนื่อง | ข้อ 4.1, **4.3** | ต่อยอด series/template · ข้ามวันหยุด/สอบอัตโนมัติ |
| 8 | **ปฏิทินการศึกษาแยกตามหลักสูตร/ชั้นปี** | ใน 1 ปีการศึกษามีปฏิทินได้หลายชุด — เปิด/ปิดเทอม + ช่วงสอบต่างกันตามหลักสูตร (ป.ตรี/โท/เอก/นานาชาติ) และตามชั้นปี (ปี 1-2 vs 3-4) · เคาะทำแบบเต็ม "ปฏิทินแยกตามกลุ่ม" | **2.4** | ⚠️ เพิ่ม `academic_calendars` · `terms` ย้ายไปสังกัดปฏิทิน · `AcademicCalendar` service resolve ตามหลักสูตร/ชั้นปี · กระทบ Branch A (บล็อก slot สอบ/ปิดเทอม) |

---

## สรุปประวัติการเปลี่ยนแปลงเอกสาร (Changelog)

### V3 → V4 (รอบปัจจุบัน — จากการประชุมลูกค้า)

| รายการ | สิ่งที่เพิ่ม/ปรับปรุงใน V4 |
|--------|-----------------|
| ข้อ 4.1 | เพิ่มฟังก์ชันหลัก: สร้างกิจกรรมแบบลากช่วงวันที่, เลือกหัวข้อจาก dropdown, แสดงเวลาแก้ไขล่าสุด |
| ข้อ 2.4 | **เพิ่ม** ปฏิทินการศึกษาแยกตามหลักสูตร/ชั้นปี (academic_calendars + terms ต่อปฏิทิน) — เคาะทำแบบเต็ม |
| ข้อ 4.3 | **ใหม่** — การสร้างกิจกรรมแบบลากช่วงวันที่ (Date Range Bulk Entry) สำหรับวิชาปฏิบัติต่อเนื่อง |
| ข้อ 5.1 | **เพิ่ม** หลักสูตรประเภท "นับงานบริการวิชาการอย่างเดียว" (ไม่นับชั่วโมงทำการปกติ) |
| ข้อ 5.5 | ⚠️ **กลับทิศจาก V3** — หัวหน้าวิชาจัดกลุ่มนักศึกษาตั้งแต่แรก โดยอิงจำนวน นศ. ในชั้นปี ไม่ใช่ capacity วิชา |
| ข้อ 5.6 | **ใหม่** — หัวข้อกิจกรรมสำเร็จรูป (Activity Topic Templates แบบ Dropdown) |
| ข้อ 6.1 | **ใหม่** — ผู้บริหารคือหัวหน้าภาควิชา + สิทธิ์กำหนดตำแหน่งหัวหน้าภาคจำกัดเฉพาะ executive |
| ข้อ 6.2 | **ใหม่** — อาจารย์กรอกสัดส่วนภาระงาน PA ด้วยตนเอง |
| ข้อ 7.1 | **ใหม่** — การสลับเวร/แลกคลาสผ่านหัวหน้าวิชา + แสดงเวลาแก้ไขล่าสุด (timestamp) |
| ข้อ 13 | **ใหม่** — สรุปรวมข้อกำหนดใหม่ 7 ข้อจากการประชุมลูกค้า |

### V2 → V3 (ประวัติเดิม)

| รายการ | สิ่งที่เพิ่ม/ปรับปรุงใน V3 |
|--------|-----------------| 
