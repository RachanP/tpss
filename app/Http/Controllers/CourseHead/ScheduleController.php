...
class ScheduleController extends Controller
{
    ...
    private function runRealtimeCheck(
        ...
        // ── Department mismatch warning (ไม่ block) ────────────────────────
        $deptWarnings = $this->checkInstructorDepartmentWarnings($courseOffering, $instructorIds);
        $warnings     = array_merge($warnings, $deptWarnings);

        // ── Capacity exceeded warning ──────────────────────────────────────
        if (! empty($data['capacity_required']) && ! empty($studentGroupIds)) {
            $selectedCount = (int) $courseOffering->studentGroups()
                ->whereIn('id', $studentGroupIds)
                ->sum('student_count');

            if ($selectedCount > (int) $data['capacity_required']) {
                $warnings[] = [
                    'type'    => 'capacity_exceeded',
                    'field'   => 'capacity_required',
                    'message' => "จำนวนผู้เรียนที่เลือก ({$selectedCount} คน) เกินความจุ ({$data['capacity_required']} คน)",
                ];
            }
        }
    ...
    ): RedirectResponse
    {
        ...
        $allWarnings = collect($conflicts)
            ->pluck('message')
            ->merge(collect($deptWarnings)->pluck('message'))
            ->unique()
            ->values()
            ->all();
        ...
        return redirect()
            ...
            ->with('schedule_conflict_warning', $allWarnings);
    }
